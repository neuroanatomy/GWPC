...
ir <- sapply(1:N, function(i){set.seed(i); sample(nrow(X))})
...
for (hemi in c("lh", "rh")) {
  ...
  foreach (i = 1:N, .combine='c') %dopar% {
    ...
    id <- nr[i]
    Xr <- X
    Xr[, contrast_ASD!=0] <- X[ir[,i], contrast_ASD!=0]
    mod <- .lm.fit(Xr, t(gwc.aparc))
    ef.aparc <- contrast_ASD %*% coef(mod)

    ef <- double(nrow(gwc))
    ef[aparc.mask] <- ef.aparc

    freesurferformats::write.fs.mgh(file.path(out.dir, sprintf("%s.ASDr%d.mgh", hemi, id)), ef)
    NULL
  }
  stopCluster(cl)
}
